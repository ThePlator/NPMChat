    ...
    <main className="flex flex-col flex-1 h-full bg-white">
      ...
      {showUserDetails && selectedUser && (
        <div
          ...
          <div
            className="bg-white rounded-2xl border-2 border-black shadow-lg p-0 flex flex-col items-center gap-0 relative min-w-[340px] max-w-xs"
      ...
      {enlargedImage && (
        <div
          ...
          <div className="relative">
            ...
            />
            <button
              className="absolute top-2 right-2 bg-white border border-black rounded-full w-8 h-8 flex items-center justify-center text-xl font-bold"
              onClick={(e) => {
                e.stopPropagation();
            ...
            <a
              href={enlargedImage}
              download="image.jpg"
              className="absolute bottom-2 right-2 bg-white border border-black rounded-full w-8 h-8 flex items-center justify-center text-xl font-bold"
              onClick={(e) => e.stopPropagation()}
      ...
      {/* Header */}
      <div className="flex items-center gap-3 px-4 py-3 border-b-2 border-black bg-white sticky top-0 z-10">
        {/* Back button for mobile */}
        <button
          className="md:hidden mr-2 p-2 rounded-full border-2 border-black bg-[#b39ddb] text-black hover:bg-[#39ff14] focus:outline-none"
          onClick={onBack}
        ...
        <div
          className="w-10 h-10 rounded-full bg-[#b39ddb] flex items-center justify-center text-xl font-extrabold border-2 border-black cursor-pointer hover:opacity-80"
        ...
        <div className="flex flex-col">
          <span
            className="text-lg font-extrabold text-black cursor-pointer hover:text-[#39ff14]"
            onClick={() => setShowUserDetails(true)}
            title="View profile">
      ...
      {/* Messages */}
      <div className="flex-1 overflow-y-auto px-4 py-4 space-y-4 bg-[#f3e8ff]">
      ...
      {/* Input */}
      <form
        className="flex items-center gap-2 border-t-2 border-black px-2 py-2 bg-white sticky bottom-0 z-10 relative"
        onSubmit={(e) => {
          e.preventDefault();
          handleSend();
        }}>
        <button
          type="button"
          className="px-2 py-2 text-xl border-2 border-black rounded-full bg-white hover:bg-[#b39ddb]"
        ...
        <label
          className="px-2 py-2 cursor-pointer border-2 border-black rounded-full bg-white hover:bg-[#b39ddb] flex items-center justify-center"
          title="Attach image">
          <input
            type="file"
            accept="image/*"
        ...
        {image && (
          <div className="relative flex items-center">
            <img
              src={image}
              alt="preview"
              className="w-12 h-12 object-cover rounded border-2 border-black mr-2"
            />
            <button
              type="button"
              className="absolute top-0 right-0 bg-white border border-black rounded-full w-5 h-5 flex items-center justify-center text-xs"
        ...
        <input
          ...
          className="flex-1 px-3 py-2 border-2 border-black rounded-full font-medium text-base bg-[#f3e8ff] focus:bg-white focus:outline-none focus:border-[#39ff14] text-black placeholder:text-gray-400"
        />
        <button
          type="submit"
          className="px-4 py-2 bg-[#39ff14] text-black text-base font-bold rounded-full border-2 border-black transition-all duration-100 hover:bg-[#b39ddb] hover:text-white hover:scale-105 shadow-sm">
          Send
        </button>
      </form>
      {loadingMessages && (
